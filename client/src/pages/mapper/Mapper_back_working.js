    ...
    {/*<Layer type="symbol" id="treesmap" layout={layoutLayer} images={images}>*/}
     ...
     <Layer type="circle" id="treesmap" 
      paint={{ 'circle-radius': { 'base': 1.75,
          ...
          >
      {treeListMap && treeListMap.features.map(tree => {
      // console.log('tree----',tree);
        // const statusColor = tree.status;
        // const iconColor = createTreeMarker(tree);
        return (<Feature
          key={tree.id}
          onClick={() => {handleClickedTree(tree);}}
          onHover={() => {setCurrentTree(tree);setShowTreePopper(true);}}
      ...
      {showTree && currentTree &&  (<TreeData tree={currentTree} icon={iconCurrent} showTree={showTree} setShowTree={setShowTree}/>)}
...
const clusterMarker = (coordinates) => (
  <TreeMarker coordinates={coordinates} >
    C
  </TreeMarker>
);
