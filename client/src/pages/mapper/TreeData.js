...
import { Button, ButtonToggle, Modal, ModalHeader, ModalBody, ModalFooter, 
  Progress, ButtonGroup, Label, Input, Col, Row } from 'reactstrap';
...
export function TreeData({currentTreeId, iconCurrent, showTree, setShowTree}) {
  ...
  console.log('treeHistory', treeHistory,  'tree POST FETCH DDDDDDDATA treeInfo \n\n\n\n');
  // console.log('treeInfo', treeInfo, 'tree POST FETCH DDDDDDDATA treeInfo \n\n\n\n');
  const { id_tree, common, scientific, planted, health, 
      address, city, country, neighborhood, lat, lng, owner, ref, who, notes } = tree || {};


    const { id_treehistory,
    watered, mulched, weeded, staked, braced, pruned, 
    datevisit, comment, volunteer  } = treeHistory || {};
  ...
  const [dateVisitMaintenance, setDateVisitMaintenance] = useState(datevisit || null);
  ...
  const image = (!tree || tree.pictureurl === "None" || tree.pictureurl === null || tree.pictureurl === undefined) ?  'icon' : tree.pictureurl;
  const tree__imageclass = (!tree || tree.pictureurl === "None" || tree.pictureurl === undefined) ? 'tree__icon' : 'tree__image';
  ...
  const onCheckboxBtnClick = (selected) => {
    ...
    (statusSelected.includes('watered')) ? setWater('watered') : setWater('water');
    (statusSelected.includes('weeded')) ? setWeed('weeded') : setWeed('weed');
    (statusSelected.includes('mulched')) ? setMulch('mulched') : setMulch('mulch');
  ...
  <Modal isOpen={showTree} toggle={toggle} className='tree__modal'>
    ...
    <ModalHeader toggle={toggle}>
      <div className="flex-grid-three text-left">
        {common && <div><h3>{common}</h3></div>}
        {scientific && <div><h4>{scientific}</h4></div>}
        {planted && <div><h5>Planted: {moment(planted).format('MMMM Do YYYY')}</h5></div>}
      </div>
    </ModalHeader>
    
    <ModalBody>
    <div className="flex-grid tree_history-list text-center"><h4>Overall Health</h4></div>
    <div className="tree__status text-center">
      <input type="range" min="0" max="100" value={sliderValue} 
      onBlur={handleSubmit} className="slider" id="myRange" onChange={changeSlider}/>
      <h3><span id={sliderValue}>{overallHealth.toUpperCase()}</span></h3>
    </div>

    <div className="flex-grid border-top">
      <div className="text-center tree_history-list"><h4>Tree Notes</h4></div>
      <div className="flex-grid tree__status">
      <textarea className="form-control tree__status__textarea" 
        ref={notesRef} id="notes" aria-label="Tree Notes"
        defaultValue={notes}></textarea>
     </div>
    </div>
    
    <div className="flex-grid border-top">
      <div className="text-center tree_history-list"><h4>Tree Maintenance</h4></div>
      <div className="tree__status__maintenance-header" >
        <Button className="tree__status__maintenance-btn-header btn-lg btn-block" color="success" 
          onClick={() => setShowDoMaintanaince(!showDoMaintanaince)} > 
          Water the Trees! 
        </Button>
        {!showDoMaintanaince &&  <img className="tree__status__maintenance-header__img" src="assets/images/trees/wateringcanright2.svg" />}
        {showDoMaintanaince &&  <img className="tree__status__maintenance-header__img" src="assets/images/trees/wateringcandown.svg" />}      
      </div>
      ...
      <div className="tree__status__maintenance">
        ...
        <div className="flex-grid tree__status">
          <div className="flex-grid text-center">
            Volunteer Name:
            <input ref={volunteerRef} placeholder="Volunteer" onBlur={handleVolunteer} className="tree__status__input" id="volunteerName"/>
          </div>
        </div>

        <div className="tree__status__maintenance-buttons">
          <Button className="tree__status__maintenance-btn btn-sm" color="success" onClick={() => onCheckboxBtnClick('water')} active={statusSelected.includes('water')}  >
            <img src={`assets/images/trees/${water}.svg`} />
            <h4>{water}</h4>
          </Button>

        
          <Button className="tree__status__maintenance-btn btn-sm" color="success" onClick={() => onCheckboxBtnClick('mulch')} active={statusSelected.includes('mulch')}>
            <img src={`assets/images/trees/${mulch}.svg`} />
            <h4>{mulch}</h4>
          </Button>{' '}

        
          <Button className="tree__status__maintenance-btn btn-sm" color="success" onClick={() => onCheckboxBtnClick('weed')} active={statusSelected.includes('weed')}>
            <img src={`assets/images/trees/${weed}.svg`} />
            <h4>{weed}</h4>
          </Button>{' '}

          <Button className="tree__status__maintenance-btn btn-sm" color="success" onClick={() => onCheckboxBtnClick('stake')} active={statusSelected.includes('stake')}>
            <img src={`assets/images/trees/${stake}.svg`} />
            <h4>{stake}</h4>
          </Button>{' '}

        
          <Button className="tree__status__maintenance-btn btn-sm" color="success" onClick={() => onCheckboxBtnClick('brace')} active={statusSelected.includes('brace')}  >
            <img src={`assets/images/trees/${brace}.svg`} />
            <h4>{brace}</h4>
          </Button>

        
          <Button className="tree__status__maintenance-btn btn-sm" color="success" onClick={() => onCheckboxBtnClick('prune')} active={statusSelected.includes('prune')}  >
        ...
        <div className="flex-grid tree__status">
          ...
          <div className="flex-grid text-center">
            Maintenance Comment:
            <textarea className="form-control tree__status__textarea" onBlur={handleComment}
            ref={commentRef} placeholder="Maintenance Comment" id="comment" aria-label="Tree Notes" />
          </div>
    ...
    <div className="flex-grid border-top">
      {dateVisitMaintenance && <div className="text-center tree_history-list"><h4>Tree Visit History</h4></div>}
      
      {dateVisitMaintenance && (
        <div className="flex-grid tree__indent">
          {dateVisitMaintenance} tree visit by {volunteerRef.current ? volunteerRef.current.value : 'Volunteer'}
        </div>
      )}
      ...
      {treeHistory && treeHistory.map(history => {
        const {id_treehistory, id_tree, 
          watered, mulched, weeded, staked, braced, pruned, 
          datevisit, comment, volunteer } = history || {};

        return (
          <div className="flex-grid">
            <div className="flex-grid tree__indent">
              {moment(datevisit).format('MMMM Do YYYY')} tree visit by {volunteer || 'volunteer'}
            </div>
            ...
            <div className="flex-grid tree__indent">
              <span>
                Maintenance Done: {watered && 'watered'} {mulched && 'mulched'} {weeded && 'weeded'} {staked && 'staked'} {braced && 'braced'} {pruned && 'pruned'}
              </span>
            </div>
          </div>
        )
