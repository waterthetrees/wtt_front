...
export function Tree({tree, today, modal, toggle, lat,lng}) {
  const {label, icon, commonname, treetag, plant_date, owner,buttonLabel,className} = tree;
  ...
  <Modal isOpen={modal} toggle={toggle} className={className}>
    ...
    <ModalHeader toggle={toggle}>{label} 
      {treetag}{getmedal && <img className='Tree__icon' src='assets/images/trees/medal.svg' height="50px"/>}
      <div className="flex-grid-three text-left">
      <div><h3>{tree.commonname}</h3></div>
      <div><h4>{tree.botanicalname}</h4></div>
    ...
    <ModalBody>
      <div className="flex-grid-thirds tree__col status">
        ...
        <div className="progressbar" max="100" value={progressNumber} >
        </div>
        <div>
      ...
      <div className="flex-grid tree__col  tree__col text-center">
        {(statusSelected.length > 0) && (
          <span>
            <h5>status: {(statusSelected.length > 0) ? statusSelected.join(', ') : 'good'} </h5>
          </span>)
        }
        {(issuesSelected.length > 0) && (<span>
            <h5>issues: {(issuesSelected.length > 0) ? issuesSelected.join(', ') : 'healthy'} </h5>
          </span>)
        }
      ...
      <div className="flex-grid tree_history-list border-top"><h5>Maintanence</h5></div>
      <div className="flex-grid-buttons tree__status  ">
        <div className="text-center">
          <Button className="tree__status-btn" color="success" onClick={() => {onCheckboxBtnClick('watered');addWater('watered')}} active={statusSelected.includes('watered')}  >
        ...
        <div className="text-center">
          <Button className="tree__status-btn" color="success" onClick={() => onCheckboxBtnClick('weeded')} active={statusSelected.includes('weeded')}>
        ...
        <div className="text-center">
          <Button className="tree__status-btn" color="success" onClick={() => onCheckboxBtnClick('mulch')} active={statusSelected.includes('mulch')}>
        ...
        <div className="text-center">
          <Button className="tree__status-btn" color="success" onClick={() => onCheckboxBtnClick('prune')} active={statusSelected.includes('prune')}  >
        ...
        <div className="text-center">
          <Button className="tree__status-btn" color="success" onClick={() => onCheckboxBtnClick('stakes')} active={statusSelected.includes('stakes')}>
        ...
        <div className="flex-grid tree_history-list border-top"><h5>Issues</h5></div>
      <div className="flex-grid-buttons tree__status  ">
        <div className="text-center">
          <Button className="tree__status-btn" color="danger" onClick={() => onIssuesClick('insects')} active={issuesSelected.includes('insects')}>
        ...
        <div className="text-center">
          <Button className="tree__status-btn" color="danger" onClick={() => onIssuesClick('broken')} active={issuesSelected.includes('broken')}>
        ...
        <div className="text-center">
          <Button className="tree__status-btn" color="danger" onClick={() => onIssuesClick('electric lines')} active={issuesSelected.includes('electric lines')}>
      ...
      <div className="flex-grid tree_history-list border-top"><h5>History</h5></div>
      {dateWatered && (
    ...
    <ModalFooter>
      <div className="flex-grid tree__col  tree__col text-center">
        <div><h4>{tree.owner}</h4></div>
        <div><h5>{tree.address} {tree.street}</h5></div>
        <div><h5>{tree.treecity} {tree.treecountry}</h5></div>
        <div><h5>{tree.plantingdistrict}</h5></div>
        <div><h5>Lat/Lng: </h5></div>
        <div><h5>[{tree.latitude},{tree.longitude}]</h5></div>
